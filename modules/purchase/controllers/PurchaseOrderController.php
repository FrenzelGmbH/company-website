...
class PurchaseOrderController extends AppController
{
  ...
  public function actionCreateRequest()
  {
    ...
    $model = new PurchaseOrder;
    $model->contact_id = \Yii::$app->user->identity->CurrentContactId;
    
    if($model->save())
    {
    	$groupModel = new PurchaseOrderGroup;
    	$groupModel->contact_id = \Yii::$app->user->identity->CurrentContactId;
  ...
  /**
	...
	public function actionWindow($win, $id=NULL,  $mainid=NULL)
	{
		...
		{
			$showform = '/purchase-order/windows/_form_'.$winparams[0];
	...
	{
		if (($model = PurchaseOrderGroup::findOne($id)) !== null) {
			return $model;
		} else {
			throw new NotFoundHttpException('The requested page does not exist.');
		}
