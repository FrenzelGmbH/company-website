...
class PartyController extends AppController
{
	...
	/**
	 * grep layout from app module
	 * @var [type]
	 */
	public $layout = \app\modules\app\controllers\AppController::adminlayout;
	 ...
	 * @return [type]      [description]
	 */
	public function actionPartywindow($win, $id=NULL,  $mainid=NULL)
	{
		$winparams = explode('_',$win);
		...
		{
			$showform = '/party/windows/_form_'.$winparams[0];
	...
	public function actionJsonlist($search = NULL,$id = NULL)
	{
		...
		{			
			if(!is_null($id))
			{
				$clean['results'] = ['id'=> $id,'text' => Party::findOne($id)->organisationName];
			}else
			{
